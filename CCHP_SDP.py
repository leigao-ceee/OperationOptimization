...
path = r'D:\Lei\work\CombinedSystemOperation\CCHP_ceee_office\cchp_data.xlsx'
...
def dema_Ht_link(m, Scen, t):
    if t == m.Stag.first():
        return sum(m.block[t].Heat[SSHt, Scen] for SSHt in model.SSHt) + 0 - m.block[t].StSH[Scen] >= model.DeHt[
            t, Scen]
    else:
        return sum(m.block[t].Heat[SSHt, Scen] for SSHt in model.SSHt) + m.block[t - 1].StSH[Scen] - m.block[t].StSH[
            Scen] >= model.DeHt[t, Scen]
...
def dema_El_link(m, Scen, t):
    if t == m.Stag.first():
        return sum(m.block[t].Elec[SSEl, Scen] for SSEl in model.SSEl) + m.block[t].ElGd[Scen] + 0 - m.block[t].StSE[
            Scen] >= model.DeEl[t]
    else:
        return sum(m.block[t].Elec[SSEl, Scen] for SSEl in model.SSEl) + m.block[t].ElGd[Scen] + m.block[t - 1].StSE[
            Scen] - m.block[t].StSE[Scen] >= model.DeEl[t]
